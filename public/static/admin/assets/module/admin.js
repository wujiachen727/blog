...
layui.define(["layer"], function (f) {
    ...
    n.events = {
        flexible: function () {
            ...
            n.strToWin(i.window).layui.admin.popupRight(h.extend({id: "layer-theme", url: i.url || "page/tpl/tpl-theme.html"}, n.parseLayerOption(i)))
        }, note: function () {
            var i = n.util.deepClone(h(this).data());
            n.strToWin(i.window).layui.admin.popupRight(h.extend({id: "layer-note", url: i.url || "page/tpl/tpl-note.html"}, n.parseLayerOption(i)))
        }, message: function () {
            var i = n.util.deepClone(h(this).data());
            n.strToWin(i.window).layui.admin.popupRight(h.extend({id: "layer-notice", url: i.url || "page/tpl/tpl-message.html"}, n.parseLayerOption(i)))
        }, psw: function () {
            var i = n.util.deepClone(h(this).data());
            n.strToWin(i.window).layui.admin.open(h.extend({id: "layer-psw", title: "修改密码", shade: 0, url: i.url || "page/tpl/tpl-password.html"}, n.parseLayerOption(i)))
        }, logout: function () {
            ...
            n.strToWin(p.window).layui.layer.confirm(p.content || "确定要退出登录吗？", h.extend({title: "温馨提示", skin: "layui-layer-admin", shade: 0.1}, n.parseLayerOption(p)), function () {
                i()
            })
        }, open: function () {
            var i = n.util.deepClone(h(this).data());
    ...
    n.tips = function (i) {
        return k.tips(i.text, i.elem, {
            tips: [i.direction || 1, i.bg || "#191a23"], tipsMore: i.tipsMore, time: i.time || -1, success: function (p) {
    ...
    h(document).on("mouseenter", "*[lay-tips]", function () {
        var i = h(this);
        n.tips({elem: i, text: i.attr("lay-tips"), direction: i.attr("lay-direction"), bg: i.attr("lay-bg"), offset: i.attr("lay-offset"), padding: i.attr("lay-padding"), color: i.attr("lay-color"), bgImg: i.attr("lay-bgImg"), fontSize: i.attr("lay-fontSize")})
    ...
    h(document).on("click", ".layui-collapse>.layui-colla-item>.layui-colla-title", function () {
        ...
        s.children(".layui-colla-icon").html("&#xe602;").css({"transition": "all .3s", "transform": "rotate(" + (p ? "90deg" : "0deg") + ")"});
        if (q.attr("lay-shrink") === "_all") {
            ...
            r.siblings(".layui-colla-title").children(".layui-colla-icon").html("&#xe602;").css({"transition": "all .3s", "transform": "rotate(0deg)"})
        }
    });
    ...
    d = h.extend({
        pageTabs: true, cacheTab: true, openTabCtxMenu: true, maxTabNum: 20, tableName: "easyweb-iframe", apiNoCache: true, ajaxSuccessBefore: function (p, i, q) {
